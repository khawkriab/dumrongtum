...
import { complaint, headComplaint, sub_complaint, complaintToRight, complaintToCenter, selectComplaint } from '../Components/Static/static';
import { InputForm, RadioForm, RadioFormGroup, SelectFormGroup, InputRow, Textarea, InputIdentifi, InputID, SelectName, RadioeDePart, InputFormGroup } from '../Components/Form/Input';
...
export default class AdminAddCompaint extends Component {
	constructor(props) {
		super(props);
		...
		return (
			<PanelRight>
				...
				<PanelContent>
					...
					<PanelWhi>
						<button
							className={step == 0 ? 'box btn-orange-absol -hiddle' : 'box btn-orange-absol'}
	...
	Step1() {
		const { step, title, type, subtype, complaint_channel, subject } = this.state;
		return (
			<div className={step !== 0 ? 'd-none' : 'd-grid'}>
				<span>หัวข้อร้องทุกข์</span>

				<InputFormGroup title="หัวข้อร้องทุกข์" onChange={this.updatetypeInput} name={'subject'} value={subject} />

				<RadioFormGroup title="ประเภทเรื่องหลัก" name={'type'} currentItem={type} allItem={headComplaint[1].data} onChange={this.updatetypeInput} xs={5} />
				<SelectFormGroup title="ประเภทเรื่องย่อย" name={'subtype'} currentItem={subtype} allItem={sub_complaint[0][type].data.map((e) => ({ label: e.value, value: e.value }))} onChange={this.updatetypeInput} />
				<RadioFormGroup title="ช่องทางการร้องเรียน" name={'complaint_channel'} currentItem={complaint_channel} allItem={headComplaint[2].data.map((e) => ({ label: e.label, value: e.label }))} onChange={this.updatetypeInput} xs={5} />
